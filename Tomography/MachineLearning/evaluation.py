...
try:
    ...
    jl.seval('using Pkg; Pkg.add(url="https://github.com/marcsgil/BayesianTomography.jl.git")')
    jl.seval('using BayesianTomography')

jl.seval("""
function complex_representation(xs)
    d = Int(√(size(xs, 2) + 1))
    inv_d = 1 / d
    basis = gell_man_matrices(d, include_identity=false)
    ρs = stack(ρ -> linear_combination(ρ, basis), eachslice(xs, dims=1))
    Threads.@threads for n ∈ axes(ρs, 3)
        for m ∈ axes(ρs, 1)
            ρs[m, m, n] += inv_d
        end
    end
    permutedims(ρs, (3, 2, 1))
end
""")
