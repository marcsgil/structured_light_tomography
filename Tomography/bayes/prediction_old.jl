...
file = h5open("Data/ExperimentalData/Old/ExperimentalResults/UFMG/config.h5", "r")
...
##
file = h5open("Data/ExperimentalData/Old/ExperimentalResults/UFMG/coefficients.hdf5")
coeffs = read(file["order$order"])
close(file)
nobs = map(x -> 2^x, 6:12)
idxs = 1:50
fids = fill(NaN, length(nobs))
srcs = ["Data/ExperimentalData/Old/ExperimentalResults/UFMG/Order$order/$i.tif" for i ∈ 0:49]
config = "Data/ExperimentalData/Old/ExperimentalResults/UFMG/config.h5"
